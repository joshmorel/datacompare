...
def sqlite_connection(request):
    ...
    try:
        with sql_connection.cursor() as sql_cursor:
            sql_cursor.execute(
                "CREATE TABLE t1(Nums INT NOT NULL, Chars VARCHAR(255), Extra VARCHAR(255), SomeDate DATETIME)")
            sql_cursor.execute("INSERT INTO t1 VALUES (?,?,?,?)", 1, 'left', 'one', '2014-04-01')
            sql_cursor.execute("INSERT INTO t1 VALUES (?,?,?,?)", 2, 'b', 'two', '2014-04-02')
            sql_cursor.execute("INSERT INTO t1 VALUES (?,?,?,?)", 3, 'c', 'three', '2015-04-02')
